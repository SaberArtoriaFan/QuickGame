...
namespace Saber.UI
{
    public static class UIBehaviorUtil
    {
        //通过写子控件修改自身属性的方法
        //#region ChangeSelf
        public static void SetSelfActive(this UIBehavior behavior, bool IsActive)
        {
            behavior.RT.gameObject.SetActive(IsActive);
        }
        public static void SetRectSize(this UIBehavior behavior, float width, float hight)
        {
            RectTransform rectTransform = behavior.RT;
        ...
        public static void SetRaycastTarget(this UIBehavior behavior, bool value)
        {
            MaskableGraphic maskableGraphic = behavior.RT.GetComponent<MaskableGraphic>();
        ...
        public static void SetPivot(this UIBehavior behavior, int x, int y)
        {
            RectTransform rectTransform = behavior.RT;
            if (rectTransform != null)
            {
        ...
        //    }
        //}
        public static void ChangePosition(this UIBehavior behavior, Vector3 pos, bool isLoacl = true)
        {
            if (isLoacl)
                behavior.RT.localPosition = pos;
            else
                behavior.RT.position = pos;
        }
        public static bool IsButtonActive(this UIBehavior behavior)
        {
            Button button = behavior.RT.GetComponent<Button>();
            if (button != null)
            {
